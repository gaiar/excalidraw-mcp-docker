...
function convertRawElements(els: any[]): any[] {
  ...
  const converted = convertToExcalidrawElements(withDefaults, { regenerateIds: false }).map(
    (el: any) =>
      el.type === 'text' ? { ...el, fontFamily: (FONT_FAMILY as any).Excalifont ?? 1 } : el,
  );
  ...
  const processedDraw =
    deleteIds.size > 0
      ? drawElements.map((el: any) =>
          deleteIds.has(el.id) || deleteIds.has(el.containerId) ? { ...el, opacity: 1 } : el,
        )
      : drawElements;
      ...
      try {
        ...
        if (renderedSvg) {
          ...
          if (vbAttr && vbAttr.length === 4) {
            const [x, y, w, h] = vbAttr as [number, number, number, number];
    ...
    const doStream = async () => {
      ...
      if (streamRestoreId) {
        ...
        if (base && streamDeleteIds.size > 0) {
          base = base.filter(
            (el: any) => !streamDeleteIds.has(el.id) && !streamDeleteIds.has(el.containerId),
          );
        }
      }
...
export function ExcalidrawAppCore({ app }: { app: App }) {
  ...
  const mountEditor =
    displayMode === 'fullscreen' && inputIsFinal && elements.length > 0 && editorReady;
  useEffect(() => {
    ...
    const settle = async () => {
      ...
      if (sceneElements?.length) {
        const { elements: fixed } = restore({ elements: sceneElements }, null, null, {
  ...
  useEffect(() => {
    ...
    if (initCtx?.containerDimensions?.height)
      setContainerHeight(initCtx.containerDimensions.height);
  ...
  return (
    <main
      ...
      style={
        displayMode === 'fullscreen' && containerHeight ? { height: containerHeight } : undefined
      }
    >
      {displayMode === 'inline' && (
      ...
      {!editorSettled && (
        <div onClick={undefined} style={undefined}>
          <DiagramView
            toolInput={toolInput}
